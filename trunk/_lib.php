...
include('ciscostaffing-conf.php');
...
//	session_unset();
//	session_destroy();
// lets see if this works for IE6
	$_SESSION['chrEmail'] = '';
	$_SESSION['idPerson'] = '';
	$_SESSION['chrFirst'] = '';
	$_SESSION['chrLast'] = '';
...
//	unset($_SESSION);
	...
	header("Location: ".$BF."signout.php?id=".$logoutReason);
	die();
}
...
function auth_check($sitm='',$accessLevels='') {
	...
	if(!isset($_SESSION['idPerson']) || !is_numeric($_SESSION['idPerson'])) {  // if this variable is set, they are already authenticated in this session
		...
		if ($_SESSION['dtLogin'] <= date('m/d/Y H:i:s',strtotime("-12 hours"))) {
			header("Location: ".$BF."destroy.php?reason=3");
			die();
		} 
