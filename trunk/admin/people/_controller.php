<?
	...
	switch($file_name[0]) {
		...
		case 'index.php':
			...
			$page_title = (access_check('1') ? linkto(array('address'=>'add.php','img'=>'plus_add.png','style'=>'float:left;')).'&nbsp;&nbsp;' : '').'People <span class="resultsShown">(<span id="resultCount">'.mysqli_num_rows($results).'</span> results)</span>';
			$directions = "Choose a person from the list below. Click on the column header to sort the list by that column.";
			$filters = 'Search:&nbsp;';
			$filters .= form_text(array('nocaption'=>'true','name'=>'chrSearch','caption'=>'Search Users','value'=>$_REQUEST['chrSearch'],'size'=>'10'));
			...
			$filters .= form_select($filter,array('name'=>'idStatus','id'=>'troublebox','caption'=>'-Select Status-','nocaption'=>'true','value'=>$_REQUEST['idStatus']));			
