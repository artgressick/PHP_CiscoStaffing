...
<?
						$q = "SELECT chrKEY, chrLocation AS chrRecord FROM Locations WHERE !bDeleted and idEvent = '".$_SESSION['idEvent']."' ORDER BY chrLocation";
			...
			<table width="100%" class="twoCol" id="twoCol" cellpadding="0" cellspacing="0">
				<tr>
					<td class="tcleft" style='vertical-align:top;'>
						<?=form_text(array('caption'=>'Zone Name','required'=>'true','name'=>'chrZone','size'=>'30','maxlength'=>'100'))?>
...
?>
			...
			<div class='innerbody'>
				<div class='FormName'><?=linkto(array('address'=>'#','img'=>'plus_add.png','extra'=>'onclick=\'newwin = window.open("popup_person.php?d='.urlencode(base64_encode('functioncall=user_add')).'","new","width=435,height=400,resizable=1,scrollbars=1"); newwin.focus();\''))?> Zone Managers</div>
						<input type='hidden' id='idUsers' name='idUsers' value='' />
						<input type='hidden' id='chrUsers' name='chrUsers' value='' />
						<table id='ListUsers' class='List' style='width: 100%;' cellpadding="0" cellspacing="0">
							...
							<tbody>
<?			if(isset($info['idUsers']) && $info['idUsers'] != '') { 
...
<?			} ?>
						...
						<div id='ListUserstrNoUser' style='border:1px solid #999;border-top:none;height:20px; padding-top:4px; text-align:center; <?=(!isset($info['idUsers']) || $info['idUsers'] == ''?'':'display: none;')?>'>
							No Zone Managers have been added.
						</div>
