<?
	if($_POST['moveTo'] == "") { // If they changed a option on the drop down menus
		...
		if($_POST['moveTo'] == "back") {
			$_SESSION['showmatrix'] = true;
			$_POST['moveTo'] = 'index.php?showmatrixr=1&idLocation='.$_POST['idLocation'];
...
//This is a new feature added by Art - Removing staffer from location.
				if($_POST['idPerson'] != 'x') {
					...
					$_SESSION['infoMessages'][] = "The Staffer has been removed.";
				}
			} else {
				$_SESSION['infoMessages'][] = "No Scheduling options has been changed.";
			}

			if($_POST['moveTo'] == 'assignshift.php') { 
				$_POST['moveTo'] = 'assignshift.php?d='.$_POST['d'];
				header("Location: ".$_POST['moveTo']);
				die();	
			} else {
				$_SESSION['showmatrix'] = true;
