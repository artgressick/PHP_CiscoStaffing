...
<?
	$locations_query = "SELECT ID, chrLocation AS chrRecord 
		FROM Locations 
		WHERE !bDeleted AND bStaffingEnabled AND idEvent='".$_SESSION['idEvent']."' ";

	//Check to see if they are an Admin.
	if ($_SESSION['bAdmin'] != "1") {
		$locations_query .= " AND Locations.id in (select Zones.idLocation
			from ZoneManagers 
			join Zones on ZoneManagers.idZone = Zones.id
			where idPerson = '". $_SESSION['idPerson'] ."')";
	}
		
	$Locations = db_query($locations_query . " ORDER BY chrLocation","Getting Locations");
?>
...
<?
	$Shifts = db_query("SELECT ID, CONCAT(DATE_FORMAT(dDate,'%a, %b %e, %Y'),' from ',DATE_FORMAT(tBegin,'%l:%i %p'),' to ',DATE_FORMAT(tEnd,'%l:%i %p')) as chrRecord 
		FROM Shifts 
		WHERE !bDeleted AND idLocation='".$data['idLocation']."' 
		ORDER by dDate, tBegin, tEnd","Getting Shifts");
?>
...
<?
	$stations_query = "SELECT ID, CONCAT(chrStation,' (',chrNumber,')') as chrRecord 
		FROM Stations 
		WHERE !bDeleted AND idLocation='".$data['idLocation']."' ";
		
	//again we need to limit the results down for non-administrators
	if ($_SESSION['bAdmin'] != "1") {
		$stations_query .= " AND Stations.id in (select idStation 
			from ZoneStations
			join ZoneManagers on ZoneStations.idZone = ZoneManagers.idZone
			where ZoneManagers.idPerson = ".$_SESSION['idPerson'].")";
	}
	
	$Stations = db_query($stations_query . " ORDER BY chrRecord","Getting Stations");
?>
...
*/
	// The following query is for allowing staffer to scheduled multiple times per day
		...
		$q = "SELECT People.ID, People.chrKEY, People.chrFirst, People.chrLast,Shifts.dDate,
				(SELECT count(ID) FROM Schedule WHERE idPerson=People.ID AND idEvent='".$_SESSION['idEvent']."') as intAssigned
			FROM People
			JOIN Schedule_Requested AS SR ON SR.idPerson=People.ID AND SR.idShift='".$data['idShift']."' AND SR.idEvent='".$_SESSION['idEvent']."'
			JOIN Shifts ON Shifts.ID=SR.idShift
			WHERE People.idPersonStatus=3
...
?>
							...
							<tr class='<?=($count++%2 ? 'ListOdd' : 'ListEven')?>'>
								<td style='cursor: default;'><input type='radio' name='idPerson' value='x' id='idPersonx' title='Remove Staffer Needed'/></td>
								<td style='cursor: default;'>&nbsp;</td>
								<td colspan='3' style='cursor: default;'><label for='idPersonx' style='cursor: pointer;'><strong>Remove Staffer</strong></label></td>
							</tr>
							</tfoot>
						</table>
<?
...
?>
				...
				<?=form_button(array('type'=>'submit','value'=>'Add And Return','extra'=>'onclick="document.getElementById(\'moveTo\').value=\'assignshift.php\';"'))?> &nbsp;&nbsp; <?=form_button(array('type'=>'submit','value'=>'Add and Go to Matrix','extra'=>'onclick="document.getElementById(\'moveTo\').value=\'index.php\';"'))?> &nbsp;&nbsp; <?=form_button(array('type'=>'submit','value'=>'Go to Matrix','extra'=>'onclick="document.getElementById(\'moveTo\').value=\'back\';"'))?>
<?
	}
?>
