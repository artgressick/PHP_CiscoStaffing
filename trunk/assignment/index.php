...
//		if(!isset($_REQUEST['idZone'])) { $_REQUEST['idZone'] = ""; }
		...
		$locations_query = "SELECT ID, chrLocation
			FROM Locations 
			WHERE !bDeleted AND Locations.idEvent='". $_SESSION['idEvent'] ."' AND bStaffingEnabled ";
			
			//If they are not an Admin then we need to restrict the Locations down to a Zone.
			...
			$locations = db_query($locations_query . " ORDER BY chrLocation ASC
		","getting locations");
...
//		if($_SESSION['bZoneManager'] == 1) { $str = implode(',',$_SESSION['idZones']); }
		...
		if($_REQUEST['idLocation'] != "") {
			...
			if ($_SESSION['bAdmin'] != "1") {
				$q .= " AND Stations.id in (select idStation 
				from ZoneStations
				join ZoneManagers on ZoneStations.idZone = ZoneManagers.idZone
				where ZoneManagers.idPerson = ".$_SESSION['idPerson'].")";
			}
				
			$q .= "ORDER BY chrNumber, chrStation, idStation, Shifts.dDate, Shifts.tBegin";
...
?>
	...
	<table class='FilterBar' cellpadding="0" cellspacing="0" border="0" style='padding:10px;'>
		<tr>
			<td>
				<span style='padding: 2px 2px 0 0;'>Sort by</span> <select name="idLocation" id='idLocation' onchange='this.form.submit();'>
