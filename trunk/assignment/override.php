...
?>
						...
						<table class='Tabs' style='margin-top: 1em; margin-bottom: -3px; width: 100%;'>
							<tr>
								<td><a href='assignshift.php?d=<?=$_REQUEST['d']?>' style='padding-left:5px;padding-right:5px;'>Standard</a></td>
								<td class='Current'><a href='override.php?d=<?=$_REQUEST['d']?>' style='padding-left:5px;padding-right:5px;'>Override</a></td>
						...
						<table id='PersonList' class='List sortable' style='width: 100%' cellpadding="0" cellspacing="0">
							<thead>
							<tr>
...
<?
		...
		$q = "SELECT People.ID, People.chrKEY, People.chrFirst, People.chrLast,Shifts.dDate,
			...
			FROM People
			LEFT JOIN Schedule_Requested AS SR ON SR.idPerson=People.ID AND SR.idShift='".$data['idShift']."' AND SR.idEvent='".$_SESSION['idEvent']."'
			LEFT JOIN Shifts ON Shifts.ID=SR.idShift
...
?>
				...
				<?=form_button(array('type'=>'submit','value'=>'Add And Return','extra'=>'onclick="document.getElementById(\'moveTo\').value=\'override.php\';"'))?> &nbsp;&nbsp; <?=form_button(array('type'=>'submit','value'=>'Add and Go to Matrix','extra'=>'onclick="document.getElementById(\'moveTo\').value=\'index.php\';"'))?>
<?
	}
?>
