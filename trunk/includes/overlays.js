...
function revert() {
	...
	if(document.getElementById('troublebox')) {
		document.getElementById('troublebox').style.display='';
	}
...
//	} 
	if(document.getElementById('troublebox')) {
		document.getElementById('troublebox').style.display='none';
	}
	...
	if(ajax) {
		...
		ajax.onreadystatechange = function() { 
			if(ajax.readyState == 4 && ajax.status == 200) { 
				showNotice(id,ajax.responseText,tbl);
...
function showNotice(id, type, tbl) {
...
//				alert(JSONdata.session_name);
				document.getElementById('hour').value = JSONdata.hour;
				document.getElementById('min').value = JSONdata.min;
				document.getElementById('sec').value = JSONdata.sec;
			} 
		} 
		ajax.send(null); 
	}
...
function checkbadge(bf,badgenum) {
	ajax = startAjax();
	var address = bf + "ajax_delete.php?postType=checkbadge&badgenum="+badgenum;
	...
	if(ajax) {
		...
		ajax.onreadystatechange = function() { 
			if(ajax.readyState == 4 && ajax.status == 200) { 
//				alert(ajax.responseText);
				document.getElementById('errors').innerHTML += ajax.responseText;
