		...
		if($_REQUEST['idLocation'] != "") {
			...
			while($row = mysqli_fetch_assoc($data_result)) {
				...
				if($row['idPerson'] == '') { 
					...
					$bgcolor[$row['idStation']][$row['idShift']] = '#FFF';
				} else if($row['idPerson'] == '0') {
					$stationresults[$row['idStation']][$row['idShift']] = '-Not&nbsp;Required-';
					$bgcolor[$row['idStation']][$row['idShift']] = '#EFEFC2';
				} else {
					...
					$bgcolor[$row['idStation']][$row['idShift']] = '#EFEFC2';
					if($row['dtCheckin'] != '') {
						$bgcolor[$row['idStation']][$row['idShift']] = '#79ff8f';
					}
				}
			}
		}
...
?>
						</select>&nbsp;&nbsp;<input type='submit' name='go' value='Submit' />
<?
					if($_REQUEST['idLocation'] != '') {
?>
						&nbsp;&nbsp;<input type='button' name='excel' value='Download matrix to excel' onclick='window.location="index.php?excel=true&idLocation=<?=$_REQUEST['idLocation']?>";' />		
<?
					}
?>
					</form>
...
?>					
						<td style='background:<?=$bgcolor[$idStation][$k]?>;padding:5px;white-space:nowrap;text-align:center;border-left:1px solid #C0C1C2;border-top:1px solid #C0C1C2;'>
